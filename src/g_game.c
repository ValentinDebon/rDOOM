...
bool mousebuttons[5]; // 4 + 1, allow [-1]
...
G_Responder(event_t *ev) {
	...
	{ 
		...
		if(AM_Responder(ev)) { /* automap ate it */
			return true;
		}
	}
...
//
void
G_PlayerFinishLevel(int player) {
	player_t *p;
...
void
G_DeferedInitNew(skill_t skill,
	int episode,
	int map) {
	d_skill    = skill;
	d_episode  = episode;
